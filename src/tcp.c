...
#ifdef ENABLE_TCPREASM
static struct tcp_stream* initiate_tcp_resume(struct tcphdr* this_tcphdr, struct ip* this_iphdr, int direction) {
...
//ECN
  ...
  if (resumed_tcp) {
    ...
    add2buf(rcv, (char*)(this_tcphdr) + 4 * this_tcphdr->th_off, datalen);
	goto do_lurkers;
  }
#endif
